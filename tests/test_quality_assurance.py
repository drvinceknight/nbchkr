...
def test_check_tags_when_they_match():
    ...
    assert nbchkr.utils.check_tags_match(source_nb_node=source_nb_node, nb_node=nb_node) is True
...
def test_check_tags_when_they_do_not_match():
    ...
    nb_node = nbchkr.utils.read(nb_path=NB_PATH / "submission.ipynb")
    source_nb_node = nbchkr.utils.read(nb_path=NB_PATH / "test.ipynb")
    assert nbchkr.utils.check_tags_match(source_nb_node=source_nb_node, nb_node=nb_node) is False
